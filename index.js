...
function listOfNames(array){
  let justNames = array.map(item => item.name);
  return justNames }
...
function lotsOfArt(array){
  ...
  for (let i = 0; i < array.length; i ++) {
    if (array[i].paintings > 100){
      volumeArtists.push(array[i].name);
    }
  }
  return volumeArtists;
}
...
function artistInfo(array, name){
  let info = array.find(array => array.name === name).bio;
  return info;
}
...
function artistByCountry(array, nationality){
  const location = [];
  for (let i = 0; i < array.length; i++) {
   if(array[i].nationality === nationality && array[i].nationality !== ",") {
